...
| `RustPlusApi.Fcm.Registration` | ~97.1% | Remaining: `CredentialsStore` file-write path + an `AndroidFcmRegister` error-parse equality; the only excluded member is `SteamLoginService.TryOpenBrowser` (see the coverage exclusion list below). |
