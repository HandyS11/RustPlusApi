...
| `RustPlusApi.Fcm.Registration` | ~97.1% | Remaining: `CredentialsStore` file-write path + an `AndroidFcmRegister` error-parse equality; the two excluded members are `SteamLoginService.TryOpenBrowser` and `FcmRegistration.RegisterWithRustPlusAsync` (see the coverage exclusion list below). |
